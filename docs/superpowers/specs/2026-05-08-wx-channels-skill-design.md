...
# 不默认写入 ~/.codex/skills 或 ~/.claude/skills;已有旧副本移到同级备份目录
```
...
3. **运行时 skill 更新来源**:更新必须从 GitHub 仓库 `richardwild426/wx-channels-download` 拉取,复制到 canonical 目录 `~/.agents/skills/<name>`。不要默认写入 `~/.codex/skills` / `~/.claude/skills`;默认把这些专用目录中的旧副本移到同级备份目录。只有显式设置 `WX_SYNC_RUNTIME_SKILLS=1` 时才同步这些兼容副本。不要依赖本地工作区、软链接目标或旧拷贝,否则会出现"仓库已更新但运行时加载旧 skill"。
