...
**日期**: 2026-05-08
**状态**: 设计稿(MVP 收缩版,2026-05-08 修订),待实施
**规范依据**: [Agent Skills Specification](https://agentskills.io/specification)

---

## MVP 范围(2026-05-08 修订)

本次实施收缩为 **6 verb,集中在视频号下载与浏览**:

| 保留 | 砍掉(v0.2 再说) |
|---|---|
| `+probe`、`+download`、`+batch`、`+search`、`+list`、`+task` | `+mp`(公众号)、`+filehelper`(文件传输助手)、`+decrypt`(离线解密) |

**实测策略**: MVP 阶段**不依赖运行中的 wx_video_download 实例**,所有事实核查以上游 Go 源码为准 —— 读 `internal/api/handler.go` / `internal/channels/client.go` / `internal/api/types/types.go` 的 struct 定义。端到端冒烟测试推到 v0.2,等用户实际使用时再触发。
...
做一个本地 skill,让 agent 能驱动用户在本机或 NAS 上已经在跑的 `wx_video_download` 二进制(默认监听 `127.0.0.1:2022` 的 HTTP API),完成视频号 share URL 下载、按创作者批量下载、搜索、任务管理、列 feed 等动作。**不修改上游 Go 项目。**
...
```bash
# 仓库目录名需与 frontmatter `name` 字段保持一致
ln -s <repo-root> ~/.claude/skills/<skill-name>
```

软链接保持源版本与安装版本同步。`<skill-name>` 必须满足 agentskills 规范:小写字母 / 数字 / 连字符,且等于父目录名。
...
3. **软链接安装是否兼容 Claude Code skill 加载**:本仓库源目录名落地前未最终敲定,frontmatter `name` 与父目录名一致是硬性约束。三种应对:
   - **方案 a**: Claude Code 用 link 名做匹配(不 resolve symlink)→ 软链接可用
   - **方案 b**: 直接把源仓库目录命名为 skill name(去掉下划线 / 跟连字符规范)→ 软链接也可用
   - **方案 c**: 用 `rsync` 或 git hook 把仓库内容拷贝到 `~/.claude/skills/<name>/`

   2026-05-08 决策:**方案 b**,用户自行改源目录名 → skill name = 父目录名。安装时仍可用 symlink。
