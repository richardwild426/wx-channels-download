...
| 保留 | 砍掉 |
|---|---|
| `+install-binary`、`+run-binary`、`+probe`、`+download`、`+batch`、`+search`、`+list`、`+task` | `+mp`(公众号)、`+filehelper`(文件传输助手)、`+decrypt`(离线解密) |
...
| 2 | 自动连接 / 启动本地二进制服务 | macOS 用 LaunchAgent,Windows 用 Start-Process,Linux 用 systemd user service |
| 3 | 不替用户登录微信 / 装证书 / 点系统代理授权 | 这些动作需要用户在 GUI 完成 |
| 4 | 零状态零持久化 | 只有 SKILL.md + references;状态由 agent 用 TodoWrite 自管 |
| 5 | 不做长驻订阅 | YAGNI |
| 6 | 不封装 WebSocket 推送 | 进度走轮询 |
| 7 | NAS / 本机统一 | 一个 `WX_SERVER` 环境变量切换 |
| 8 | 不重新发明分页 / 重试 | API 自带 `next_marker`,直接文档化 |
| 9 | 不替用户做错误兜底 | 反复同错就停,交还用户判断 |
