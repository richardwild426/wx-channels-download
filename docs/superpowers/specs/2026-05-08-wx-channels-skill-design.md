...
| 3 | 自动连接 / 启动本地二进制服务 | macOS 用 LaunchAgent,Windows 用 Start-Process,Linux 用 systemd user service |
| 4 | 不替用户登录微信 / 装证书 / 点系统代理授权 | 这些动作需要用户在 GUI 完成 |
| 5 | 零状态零持久化 | 只有 SKILL.md + references;状态由 agent 用 TodoWrite 自管 |
| 6 | 不做长驻订阅 | YAGNI |
...
# 再把 <tmp-dir> 内容复制到 ~/.codex/skills/wx-channels-download 和 ~/.claude/skills/wx-channels-download
```
...
3. **运行时 skill 更新来源**:更新必须从 GitHub 仓库 `richardwild426/wx-channels-download` 拉取,再复制到 `~/.codex/skills/<name>` / `~/.claude/skills/<name>`。不要依赖本地工作区、软链接目标或旧拷贝,否则会出现"仓库已更新但运行时加载旧 skill"。
