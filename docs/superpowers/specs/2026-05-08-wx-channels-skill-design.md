...
# 再由当前智能体自己的 skill 安装机制决定安装位置
# 更新时直接替换目标 skill 目录,不保留旧副本
```
...
3. **运行时 skill 更新来源**:更新必须从 GitHub 仓库 `richardwild426/wx-channels-download` 拉取。安装位置由当前智能体自己的 skill 安装机制决定,不要在本 skill 中强制统一到某个目录,也不要同步多个运行时目录。更新时直接替换目标 skill 目录,不保留旧副本。不要依赖本地工作区、软链接目标或旧拷贝,否则会出现"仓库已更新但运行时加载旧 skill"。
